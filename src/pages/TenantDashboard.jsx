...
export default function TenantDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      await supabase.from("applications").update({ user_id: session.user.id }).eq("applicant_email", session.user.email).is("user_id", null)
  ...
  return (
    <div className="min-h-screen px-6 py-12 max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-10">
        <BrandMark />
        <button onClick={handleSignOut} className="text-sm text-neutral-500 hover:text-neutral-300">
          Sign out
        </button>
      </div>
      ...
      {!application ? (
        <div>
          ...
          <p className="mt-3 text-neutral-400 text-sm">
            You don&apos;t have an active application on file. If you applied recently, it may take a
            moment to appear.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-neutral-900 border border-white/5 rounded-lg p-5">
            ...
            <div className={`text-lg font-medium ${statusMeta?.color}`}>
              {statusMeta?.label}
            </div>
            {statusMeta?.next && (
              <p className="mt-2 text-sm text-neutral-400">{statusMeta.next}</p>
            )}
          ...
          {lease && (
            <div className="bg-neutral-900 border border-white/5 rounded-lg p-5">
              ...
              {lease.status === 'signed' && (
                <p className="mt-3 text-sm text-green-400">Lease signed ✓</p>
              )}
            </div>
          )}
            ...
            <div className="space-y-1 text-neutral-300">
              ...
              {application.desired_move_in && (
                <div className="text-neutral-400">
                  Desired move-in: {new Date(`${application.desired_move_in}T00:00:00`).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                </div>
              )}
            </div>
          </div>
